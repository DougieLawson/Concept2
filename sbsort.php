...
$union = "";
for ($i=1;$i<9;$i++) {
	$sql .= $union."SELECT elapsed, date_time, workout_name, distance FROM C2_log_row WHERE date_time BETWEEN '".$start."' AND '".$end."' AND workout_name = '".$wname[$i]."' AND elapsed = (SELECT min(elapsed) FROM  C2_log_row WHERE date_time BETWEEN '".$start."' AND '".$end."' AND workout_name = '".$wname[$i]."')";
	$union = " UNION ";
}
for ($i=1;$i<4;$i++) {
	$sql .= $union."SELECT elapsed, date_time, workout_name, distance FROM C2_log_row WHERE date_time BETWEEN '".$start."' AND '".$end."' AND workout_name = '".$namew[$i]."' AND distance = (SELECT max(distance) FROM C2_log_row WHERE date_time BETWEEN '".$start."' AND '".$end."' AND workout_name = '".$namew[$i]."')";
}
function cmp($a, $b)
{
