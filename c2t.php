...
$line=1;
$cur_date = strtotime("0001-01-01 00:00:00");
$dailytot=0;
$dailytime = 0;
$dailyrest = 0;
...
while ($row = mysql_fetch_array($rrows)) {
	extract($row);
	$wo_date = strtotime(substr($date_time,0,-8)."00:00:00");
        if (round(abs($wo_date-$cur_date)/60/60/24) > 0) {
		$line = 1;
		$cur_date = $wo_date;
	}
	$tot_dist = $distance;
...
echo "Resting metres: $rest_dist";
	$dailytot += $rest_dist;
	if ($rest_time>0) {
	$r="m\tRest time: ".secsToTime($rest_time);
	$dailyrest += $rest_time;
	$dailytime += $rest_time;
	} else{$r="m";}
echo "$r\r\nTotal: $tot_dist";
...
echo "\r\nSession total: ",$dailytot."m\r\nSession time: ".secsToTime($dailytime)."\r\nSession Rest time: ".secsToTime($dailyrest)."\r\n";
echo "</pre>";
} else {			
